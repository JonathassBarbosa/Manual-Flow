...
(function(){
  ...
  async function loadFaqForArea(role){
    ...
    if (!API_READY){
      container.innerHTML = `
        <div class="faq-setup-notice">
          <strong>⚙️ SAC ainda não conectado à planilha</strong>
          <p>Essa área vai mostrar as dúvidas reais assim que o administrador colar a URL do Google Apps Script no arquivo <code>assets/js/config.js</code>. Veja o passo a passo em <code>google-apps-script/CONFIGURAR-SAC.md</code>.</p>
        </div>`;
  ...
  openSupportBtn && openSupportBtn.addEventListener("click", () => {
    ...
    if (!API_READY){
      ...
      errEl.innerHTML = "⚙️ <strong>Ainda não conectado.</strong> Cole a URL do Apps Script em <code>assets/js/config.js</code> para ativar o login do suporte.";
      errEl.classList.add("setup");
